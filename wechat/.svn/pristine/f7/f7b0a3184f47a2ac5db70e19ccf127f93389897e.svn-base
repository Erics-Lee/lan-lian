$(document).ready(function() {
	...
	$("#enComponyInfor .xiaHua").bind("click",function(){
		...
		if($("#enComponyInfor p").eq(index).hasClass("enName")){
			...
			$("#enComponyInfor p").eq(index).css("margin","10px");
			...
			$("#enComponyInfor p").eq(index).css("margin","0");
	...
	$("#photoBtn1").bind("click",function(){
		...
		addCompony=addComponyLetterhead(rawcompanyname, rawaddress, compayname, englishaddress, contact, email,imageBase64,uniformnumber);
	...
	$(".orderInfor4 .orderInforBtn3").bind("click", function() {
		...
		if(bankAccount.code == 200) {
			...
			if(secondData.bankId == 0){
				...
				<li>\
					<p>企业账户：</p>\
					<input placeholder="请填写企业账户" value="" id="bankAccount"/>\
				</li>\
				<li>\
					<p>用户名：</p>\
				...
				<li>\
					<p>企业账户：</p>\
					<input placeholder="请填写企业账户" value="' + account + '" id="bankAccount"/>\
				</li>\
				<li>\
					<p>用户名：</p>\
					<input placeholder="用户名自动获取" id="bankUserName" value="' + componyData.rawcompanyname + '" readonly="readonly"/>\
