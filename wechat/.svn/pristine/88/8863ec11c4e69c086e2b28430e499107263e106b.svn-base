...
public class InvoiceController {
     ...
     * @throws ParseException
     */
    @RequestMapping(value = "InvoicesTasksQueryA")
    @ResponseBody
    public String InvoicesTasksQueryA(HttpServletRequest request, HttpServletResponse response) throws ParseException {
        try {
            String sessionKey = HttpClinetUtil.getSessionKey(request);
            String result = invoiceService.InvoicesTasksQueryA(sessionKey);
            return result;
        } catch (Exception e) {
            e.printStackTrace();
            return JsonUtil.error("发票数量查询失败");
    ...
    @ResponseBody
        public String invoiceQuery(HttpServletRequest request, HttpServletResponse response, String tasksid, String invoiceid) throws ParseException {
        try {
            ...
            String sessionKey = HttpClinetUtil.getSessionKey(request);
            String result = invoiceService.invoiceQuery(sessionKey,tasksid,invoiceid);
            return result;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @throws ParseException
     */
    @RequestMapping(value = "invoicePhoto")
    @ResponseBody
    public String invoicePhoto(HttpServletRequest request, HttpServletResponse response, String tasksid, String invoiceid) throws ParseException {
        try {
            if (tasksid == null && tasksid.equals("")) {
                logger.info("发票作业ID不可为空");
            }
            ...
            String sessionKey = HttpClinetUtil.getSessionKey(request);
            String result = invoiceService.invoicePhoto(sessionKey,tasksid,invoiceid);
            return result;
        } catch (Exception e) {
            e.printStackTrace();
            return JsonUtil.error("发票图片查询失败");
        }
    }
	...
	public String uploadInvoiceImageFiles(HttpServletRequest request, String base64Codes) throws Exception{
		try {
            ...
            map.put("ParentAppKey", request.getSession().getAttribute("parentAppKey").toString());
