$(document).ready(function() {
	...
	$(".uploadMask3 .maskBtn2").bind("click", function() {
		...
		var img = document.getElementsByClassName("imglist2");
		for(var i = 0; i < img.length; i++) {
			var imgList = {};
			imgList["companyno"] = companyNo;
			imgList["ImageBinaryCodes"] = img[i].src;
	...
	$(".orderInfor4 .orderInforBtn3").bind("click", function() {
		...
		if(bankAccount.code == 200) {
			...
			<li>\
				<p>企业账户：</p>\
				<input placeholder="请填写企业账户" value="' + secondData.bankAccount + '" id="bankAccount"/>\
			</li>\
			<li>\
				<p>用户名：</p>\
				<input placeholder="用户名自动获取" id="bankUserName" value="' + componyData.contact + '"/>\
			</li>'
		}
	})
...
function registerEMail(sessionKey, email) {
	...
	if(sessionKey != '') { //已经登陆成功，不需要注册了
		alert("账号已认证通过,不需验证码验证,请直接进行下一步");
		return false;
	}
...
/*添加企业的认证图片和信息*/
function addComponyLetterhead(rawcompanyname, rawaddress, compayname, englishaddress, contact, email,imageBase64,uniformnumber){
	var result="";
	var data={
